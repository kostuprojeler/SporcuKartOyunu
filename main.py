...
class KartSecmeStratejisi(ABC):
    @abstractmethod
    def kart_sec(self, uygun_kartlar): #interface - strateji seciyorsa kart secmek zorunda 
        pass
...
class Sporcu(ABC):
    ...
    def enerji(self, deger):
        if deger < 0:
            ...
            self._enerji = self.max_enerji #kapsulleme
        else:
            self._enerji = deger
...
class Kullanici(Oyuncu): #miras - oyuncu sinifinin ozelliklerini aldi
    def moral_guncelle(self, sonuc): #polimorfizm - oyuncu ve bilgisayara farkli puanlar
        if sonuc == "kazandi":
...
class Bilgisayar(Oyuncu):
    ...
    def ai_hamlesi(self, brans):
        uygun_kartlar = [kart for kart in self.kart_listesi if kart.brans == brans and not kart.kart_kullanildi_mi and kart.enerji > 0]
        if len(uygun_kartlar) == 0:
            return None
    ...
    def dosya_oku(dosya_yolu):
        ...
        with open(dosya_yolu, "r", encoding="utf-8") as dosya:
            ...
            for satir in dosya:
                satir_sayisi += 1
                satir = satir.strip()
                if not satir or satir.startswith("Branş"):
                    continue
                ...
                try:
                    ...
                    if brans_isim in BRANS_AYARLARI:
                        ...
                        yeni_kart = AktifSporcu(int(s_id), isim, takim, brans_isim, int(dayan), int(enerji), yetenek, ozellikler_dict, int(moral), reyting)
                        kart_listesi.append(yeni_kart)
                except ValueError: # try except ile hata kontrolu 
...
class OyunYonetici:
    ...
    def kartlari_dagit(self, tum_kartlar):
        for b in self.tur_sirasi:
            ...
            if len(brans_kartlari) < 8:
                return False
                
            random.shuffle(brans_kartlari)
            for i in range(4):
                self.oyuncu.kart_listesi.append(brans_kartlari[i])
            for i in range(4, 8):
                self.bilgisayar.kart_listesi.append(brans_kartlari[i])
        return True
    ...
    def tur_kazanani_belirle(self, k_kart, b_kart, ana_ozellik, guncel_tur):
        ...
        if o_puan > b_puan:
            return "kullanici", o_puan, b_puan
        if b_puan > o_puan:
            return "bilgisayar", o_puan, b_puan
        ...
        o_yet_puan, _ = YETENEK_STRATEJILERI.get(k_kart.ozel_yetenek, PasifYetenek()).uygula(0, k_kart, b_kart.ozel_yetenek, guncel_tur)
        b_yet_puan, _ = YETENEK_STRATEJILERI.get(b_kart.ozel_yetenek, PasifYetenek()).uygula(0, b_kart, k_kart.ozel_yetenek, guncel_tur)
        
        if o_yet_puan > b_yet_puan:
            return "kullanici", o_puan, b_puan
        if b_yet_puan > o_yet_puan:
            return "bilgisayar", o_puan, b_puan
            
        if k_kart._dayaniklilik > b_kart._dayaniklilik:
            return "kullanici", o_puan, b_puan
        if b_kart._dayaniklilik > k_kart._dayaniklilik:
            return "bilgisayar", o_puan, b_puan
            
        if k_kart.enerji > b_kart.enerji:
            return "kullanici", o_puan, b_puan
        if b_kart.enerji > k_kart.enerji:
            return "bilgisayar", o_puan, b_puan
            
        if k_kart.seviye > b_kart.seviye:
            return "kullanici", o_puan, b_puan
        if b_kart.seviye > k_kart.seviye:
            return "bilgisayar", o_puan, b_puan
    ...
    def tur_sonucu_uygula(self, kazanan, kaybeden, k_kart, y_kart):
        tur_puani = 10
        if k_kart.ozel_yetenek in ["Clutch Player", "Legend", "Finisher", "Captain", "Defender", "Veteran"] and self.guncel_tur >= 9:
            tur_puani = 15
        ...
        if k_kart.ozel_yetenek == "Clutch Player" and self.guncel_tur >= 9:
            tur_puani += 5
...
class KartDetayDialog(QDialog):
    ...
    def initUI(self):
        ...
        reyting_box = QFrame()
        reyting_box.setStyleSheet(f"background-color: {BRANS_AYARLARI[self.kart.brans]['renk']}; border-radius: 10px; border: none;")
        ...
        gm_layout.addWidget(QLabel(f"✅ GALİBİYET: {self.kart.kazanma_sayisi}", styleSheet="color: #4CAF50; font-weight: bold; border:none;"))
        gm_layout.addStretch()
        gm_layout.addWidget(QLabel(f"❌ MAĞLUBİYET: {self.kart.kaybetme_sayisi}", styleSheet="color: #f44336; font-weight: bold; border:none;"))
        main_layout.addWidget(gm_box)

        yetenek_box = QFrame()
        yetenek_box.setStyleSheet("background-color: #0f1117; border-left: 5px solid #FFD700; padding: 15px; border-radius: 5px;")
...
class KartGorunumu(QFrame):
    ...
    def initUI(self):
        ...
        self.setStyleSheet(f"KartGorunumu {{ background-color: {arkaplan}; border-radius: 12px; border: 3px solid {cerceve_renk}; }}")
        ...
        btn_detay.setStyleSheet(f"QPushButton {{ background-color: {self.tema_renk}; color: black; border-radius: 12px; font-weight: bold; border: none; }}")
        ...
        layout.addWidget(QLabel(f"ENERJİ: %{int(self.kart.enerji)}", styleSheet=f"color: {yazi_renk}; font-size: 9px; font-weight: bold; border: none;"))
        ...
        bar_enerji.setStyleSheet(f"QProgressBar {{ background-color: #333; border: none; border-radius: 3px; }} QProgressBar::chunk {{ background-color: {self.tema_renk if self.aktif_mi else '#666'}; border-radius: 3px; }}")
        layout.addWidget(bar_enerji)
        
        layout.addWidget(QLabel(f"MORAL: {self.kart.moral}", styleSheet=f"color: {yazi_renk}; font-size: 9px; font-weight: bold; border: none;"))
        ...
        bar_moral.setStyleSheet(f"QProgressBar {{ background-color: #333; border: none; border-radius: 3px; }} QProgressBar::chunk {{ background-color: {moral_bar_renk if self.aktif_mi else '#666'}; border-radius: 3px; }}")
        ...
        lbl_yetenek.setStyleSheet(f"background-color: {self.tema_renk if self.aktif_mi else '#444'}; color: {'black' if self.aktif_mi else '#888'}; border-radius: 5px; padding: 4px;")
...
class AnaPencere(QMainWindow):
    ...
    def initUI(self):
        ...
        self.btn_ai_goster.setStyleSheet("QPushButton { background-color: #333; padding: 12px; border: 1px solid #555; border-radius: 6px; font-weight: bold; } QPushButton:checked { background-color: #b33; }")
        ...
        self.lbl_ozellik_bilgi.setStyleSheet("color: #FFD700; background-color: rgba(26,26,26,200); padding: 10px; border-radius: 5px;")
    ...
    def arayuzu_guncelle(self):
        if self.oyun.guncel_tur >= 36:
            self.oyun_bitti_ekrani()
            return
            
        self.lbl_skor.setText(f"🏆 {self.oyun.oyuncu.isim.upper()}: {self.oyun.oyuncu.skor} | AI: {self.oyun.bilgisayar.skor}")
        ...
        if self.btn_ai_goster.isChecked():
            ...
            for kart in self.oyun.bilgisayar.kart_listesi:
                if not kart.kart_kullanildi_mi:
                    self.ai_layout.addWidget(KartGorunumu(kart, False))
        else:
            self.ai_scroll.setVisible(False)
            self.btn_ai_goster.setText("AI KARTLARINI GÖSTER")
            
        for i in reversed(range(self.kart_layout.count())):
            wi = self.kart_layout.itemAt(i)
            if wi and wi.widget():
                wi.widget().setParent(None)
    ...
    def kart_secildi(self, secilen_kart):
        ...
        if secilen_kart.ozel_yetenek == "Captain":
            for target in self.oyun.oyuncu.kart_listesi:
                if target.brans == secilen_kart.brans and target != secilen_kart:
                    target.moral = min(100, target.moral + 5)
                    
        if ai_karti.ozel_yetenek == "Captain":
            for target in self.oyun.bilgisayar.kart_listesi:
                if target.brans == ai_karti.brans and target != ai_karti:
                    target.moral = min(100, target.moral + 5)
                    
        if karar == "kullanici":
            self.oyun.tur_sonucu_uygula(self.oyun.oyuncu, self.oyun.bilgisayar, secilen_kart, ai_karti)
            sonuc = "KAZANDIN! 🎉"
        elif karar == "bilgisayar":
            self.oyun.tur_sonucu_uygula(self.oyun.bilgisayar, self.oyun.oyuncu, ai_karti, secilen_kart)
            sonuc = "KAYBETTİN! 😢"
        else:
            secilen_kart.enerji_guncelle(-3)
            ai_karti.enerji_guncelle(-3)
            sonuc = "BERABERE! (Tüm değerler eşit) 🤝"
            
        if secilen_kart.enerji <= 0:
            secilen_kart.kart_kullanildi_mi = True
        if ai_karti.enerji <= 0:
            ai_karti.kart_kullanildi_mi = True
            
        self.oyun.guncel_tur += 1
        QMessageBox.information(self, "Tur Sonucu", f"🎯 ÖZELLİK: {ozellik.upper()}\n\n{self.oyun.oyuncu.isim.upper()}: {secilen_kart.isim} (Puan: {o_guc})\nAI: {ai_karti.isim} (Puan: {b_guc})\n\nKARAR: {sonuc}")
        self.yeni_ozellik_sec()
        self.arayuzu_guncelle()

    def oyun_bitti_ekrani(self):
        o = self.oyun.oyuncu.skor
        b = self.oyun.bilgisayar.skor
        durum = f"🏆 {self.oyun.oyuncu.isim.upper()} ŞAMPİYON!" if o > b else "💀 AI KAZANDI!" if b > o else "🤝 BERABERE!"
        QMessageBox.information(self, "Sezon Bitti!", f"{self.oyun.oyuncu.isim.upper()}: {o}\nAI Puanı: {b}\n\n{durum}")
        self.close()
...
class GirisEkrani(QMainWindow):
    def __init__(self, oyun_penceresi, bilgisayar_objesi, kullanici_objesi):
        ...
        layout.addStretch()
        layout.addWidget(QLabel("<center><h1 style='color:white; font-size:48px;'>SPORCU</h1><h1 style='color:#4CAF50; font-size:40px;'>KART <span style='color:#FF9800;'>OYUNU</span></h1></center>"))
        
        input_container = QFrame()
        input_container.setStyleSheet("background-color: rgba(26, 30, 38, 200); border-radius: 15px; border: 1px solid #333;")
        input_layout = QVBoxLayout(input_container)
        input_layout.setContentsMargins(30, 30, 30, 30)
        input_layout.addWidget(QLabel("<center><h3 style='color:white;'>OYUNCU ADI</h3></center>", styleSheet="border:none;"))
        
        self.name_input = QLineEdit()
        self.name_input.setPlaceholderText("İsminizi yazın...")
        self.name_input.setFixedSize(300, 40)
        self.name_input.setStyleSheet("background-color: #0c0e12; color: white; border: 1px solid #555; padding: 5px; font-size: 14px;")
        
        input_layout.addWidget(self.name_input, alignment=Qt.AlignCenter)
        input_layout.addSpacing(20)
        input_layout.addWidget(QLabel("<center><h3 style='color:white;'>ZORLUK SEVİYESİ</h3></center>", styleSheet="border:none;"))
    ...
    def oyuna_basla(self, strateji):
        ...
        if not isim: # hata kontrolu 
            QMessageBox.warning(self, "Hata", "Lütfen bir oyuncu adı giriniz!")
            return
...
if __name__ == "__main__":
    ...
    try:
        ...
        if len(tum_kartlar) >= 24:
            ...
            QMessageBox.critical(None, "Hata", "sporcular.txt dosyasında her branştan en az 8, toplam 24 kart olmalıdır.")
